...
final class AgentLoopGatewayEndToEndTest extends TestCase
{
 ...
 public function testProfilesUsePublicTypedGatewayEndToEnd(string $profile, int $agentStages):void
 {
  ...
  $layout=new RunnerLayout($this->root);$supervisor=new ForegroundProcessSupervisor();$git=new GitCommand($supervisor,['PATH'=>(string)getenv('PATH')]);$codex=new OutcomeHost('codex');$claude=new OutcomeHost('claude');$coordinator=new ExecutionCoordinator(new AgentLoopExecutionGateway(new ExecutionGateway($this->root)),new RuntimeJournal($layout),new RunWorkspaceManager($layout,new GitWorktreeService($git),new WorkspaceCandidateHasher($git)),new CompletionEnvelopeParser(),RunnerConfig::defaults(),['codex'=>$codex,'claude'=>$claude],$supervisor,new DiagnosticLogStore($layout));
  $projection=$coordinator->run('TASK-1');self::assertTrue($projection->complete());self::assertSame($agentStages,$codex->executions+$claude->executions);self::assertSame($agentStages,$codex->probes+$claude->probes);self::assertSame($profile,$projection->profile->value);
  foreach([...$codex->prompts,...$claude->prompts]as$prompt){self::assertStringContainsString('# Current bounded execution environment',$prompt);self::assertStringContainsString('These values are bounded current execution facts, not task policy, workflow approval, or permission to widen scope.',$prompt);self::assertStringNotContainsString('binaryPath',$prompt);self::assertStringNotContainsString('OPENAI_API_KEY',$prompt);}
 }
...
{
 public int$executions=0;public int$probes=0;/** @var list<string> */public array$prompts=[];public function __construct(private readonly string$hostId){}public function id():string{return$this->hostId;}public function probe(ProcessSupervisor$p,string$w,array$e):HostAvailability{$this->probes++;return new HostAvailability($this->hostId,'fake','1',null);}public function execute(HostExecutionRequest$r,ProcessSupervisor$p):HostExecutionResult{$this->executions++;$this->prompts[]=$r->prompt;$outcome=in_array($r->roleId,['investigator','builder','hardening'],true)?'completed':'pass';$stdout='AGENT_LOOP_STAGE_RESULT '.json_encode(['outcome'=>$outcome,'summary'=>'fixture '.$r->roleId,'artifact_references'=>[],'validation_references'=>[]],JSON_THROW_ON_ERROR)."\n";return new HostExecutionResult($this->hostId,new ProcessResult(0,$stdout,'',false,'start','finish'));}
}
